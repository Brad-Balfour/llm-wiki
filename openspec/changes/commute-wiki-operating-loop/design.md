...
The intended queue-generation path is:
...
This path is intermittent, not the current reliability baseline. It created
real artifacts on July 20, then July 21-24 scheduled runs found the expected
Gmail messages and failed every artifact-writing path with the same generic
`oai_http_clients.client.ClientError`. A manual July 24 chat in the same
Project created the expected artifacts. Manual Project generation with the
currently deployed v2 sources remains the control and fallback while issue #37
is open. Once the schema, generation instructions, Voice instructions, and
managed Task prompt are deployed together for the candidate v3 trial, manual
generation with those identical v3 sources becomes the control.

The later Voice and home-maintenance boundaries have their own observed
weaknesses. In every boundary, a model statement that it wrote, retained,
loaded, validated, or processed a file is not evidence that the action
occurred.
...
- Keep scheduled queue output observable without regressing the successful
...
Project deterministically normalizes the latter to its canonical dated v3
...
valid v3 candidate whose edition date and newsletter type uniquely match the
...
This reliability loop includes scheduled/background artifact-generation
failures and Voice/session-bundle defects. It does not train the classifier.

Classifier feedback is separate: it is valid only when tied to the exact item
at the time Brad gave it and when it corrects interest, depth, or derived
routing. Presentation preferences, duplicate/prior-awareness observations,
playback errors, artifact failures, and assistant synthesis are not classifier
labels. Repeated exact corrections and high-harm false skips inform the
classifier/profile on a measured cadence; they are not prompt edits after every
commute.
...
### 13. A commute queue has one canonical playback order and rendering

After the coupled trial deployment, new queues use `tldr-commute-queue.v3`: one
ordered `items` array, one `total_items` value, an explicit per-item playback
object, deterministic `playback_text` on each item, and deterministic top-level
`sweep_playback`. The playback object contains `position`, `total`, and the
literal phrase Voice must say, for example `4 of 6`. Voice reads the pre-rendered
strings instead of composing them from several fields.
...
Already-downloaded v2 queues remain accepted by local validation and bundle
import without rewriting, but v2 is not a live generation target after the v3
trial deployment. Git history retains the retired format if it is needed for
debugging.
...
fails, it lists recent Project Library files and MAY bind only a valid v3
...
The live prompt requires an audible item envelope before any detailed-playback
content: literal
...
Bundle event order has one intentionally minimal transition rule: the first
post-sweep `item_announced` may establish any exact queue item as current, while
a `playback_transition` identifies the item being left. An explicit `next`,
`previous`, `jump`, or `repeat` constrains the following announced destination.
A later exact announcement without a transition is accepted only as incomplete
exporter evidence in a `partial` or `recovered` history. Recording both items on
the transition would duplicate identity and add a second mismatch opportunity.
...
| --- | ------------------------------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| J1  | Scheduled Task -> Project Library queue     | Direct TLDR messages delivered today and the Task prompt        | One real, parseable, dated `.txt` queue per source email is downloadable and present in the `LLM-Wiki-Car` Library. A v3 queue has one explicit N-of-M playback order plus validated item and sweep playback text. The Task Update and artifact contents are the proof.                                                                                                                                                                  | State which expected queue is absent or invalid; create no filename-only placeholder and do not use Drive or another storage connector.                                                                                                                                             | That a scheduler's “last ran” status proves a usable queue exists.                                    | Task prompt + manual acceptance record    |
| J2  | Project Library -> active reading           | One exact queue filename or unambiguous date/newsletter request | The Project normalizes the request to one canonical v3 filename in `LLM-Wiki-Car` Library, tries that filename first, then only after an exact miss MAY bind a recent candidate whose valid v3 metadata uniquely matches the requested edition date and newsletter type. It confirms the selected queue with `Reading: <M> items from <filename>.`, reads the validated `sweep_playback`, and waits for a post-sweep starting selection. | Do not begin when no uniquely matching validated queue is available; ask Brad for a date and newsletter or one exact filename.                                                                                                                                                      | A queue chosen from conversational recollection, an ambiguous request, or a similarly dated filename. | Project instructions                      |
| J3  | Active reading -> Voice playback            | The queue file already in use and Brad's request                | Before reading an item, Voice reopens the same queue file, finds the requested object in `items`, and reads its `playback_text` exactly. It does this again after an article discussion, waits after every item, and announces the end of the queue after the final item.                                                                                                                                                                | If Voice cannot reopen the same file or find the requested object, it says so and stops. A new chat needs a new queue request.                                                                                                                                                      | The queue file, item, spoken text, or Voice's ability to reopen the file.                             | Project instructions + voice-session spec |
| J4  | Voice session -> session bundle             | The queue file, Brad's words, and the visible chat              | One bundle includes the full Queue v3 JSON, the articles actually read, Brad's saves and corrections, reported problems, unresolved requests, and an honest `complete`, `partial`, or `recovered` state. Old bundles that contain an unchanged Queue v2 file remain valid local input.                                                                                                                                                   | When Brad ends the commute, reopen the named queue. After an exact miss, use another file only when one valid Queue v3 file matches the original date and newsletter. Create a partial or recovered bundle when possible; otherwise report that no downloadable bundle was created. | Item IDs, titles, URLs, Brad's intended target, missing events, or a complete history.                | Bundle schema + `session-export.md`       |
...
J5 and J6 are internal boundaries, not commands Brad must assemble or run. The
observed home-side experience is chat-mediated: Brad supplies the downloaded
original queue(s) and session bundle(s) to a maintenance/debug chat and asks the
agent to debug and process them. The agent invokes the repository's
`validate:*`, `import:*`, `retrieve:*`, and maintainer scripts as needed. A PR,
if useful changes result, is the first normal human decision point.

The narrow package scripts remain agent-invoked implementation tools and
diagnostics. There is no product requirement to remove them or collapse them
into a new human-facing CLI. `maintain:commute` may compose the full path when
appropriate without becoming a prerequisite for the supported chat workflow.
...
| `chatgpt-project/CHATGPT_CAR_QUEUE_PROMPT.md` and `chatgpt-project/session-export.md`                                          | Prompt 4.2 live         | The Instructions explain how to read a queue. `session-export.md` explains how to save notes and create the bundle. Both files are installed in `LLM-Wiki-Car`. | Prompt 4.1 was replaced and `session-export.md` was added; deployment was verified September 4, 2026. Git keeps Prompt 4.1 for rollback. |
...
| --------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `commute-queue`: Prepared Commute Queue, Queue Priority Order, Required Item Metadata                                             | Retained, revised here              | Queue v3 remains the input to J1-J4 after the coupled trial deployment; unchanged v2 artifacts remain accepted only by local validation and import.                                                                   |
...
| ---------- | ------------------------ | ----------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| J1         | `scheduled-queue-output` | Task prompt and real queue files          | Scheduled run record plus real artifact validation; identical-prompt manual run is the control/fallback |
