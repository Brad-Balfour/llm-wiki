...
Do not create a second page for a concept the wiki already covers. If the source materially improves an existing concept, update that page while preserving its useful content and provenance. If it adds no material information or useful relationship, report "no_change". A link-only change is useful only when it materially improves navigation or explains a real relationship; do not create cosmetic link churn. In each result "detail", name the affected wiki path and state whether the candidate created a page, updated a page, added useful links only, or produced no change.
