...
| P10 — generate and test the two-file queue        | P4 merged; independent of P9 and scoring changes          | R9’s exact main-file shape (`sweep_playback`, `items[].item_playback`); all other data in `-reference`; pair checks; prompt that opens only the main file at start and the reference only for requested article details; separate export joining | R9: identical-text comparison on iPhone, including default playback after a details request; correct item matching; complete exports with and without reference questions. Restore single-file v3 if needed                                                                                                     |
...
1. Add pair generation and validation, plus complete-queue reconstruction for
   bundle export. Check that the main file contains no fields beyond the sweep
   ...
   one queue and confirm both downloads work in the same Project on the phone.
2. Update the Voice and export instructions, then run the iPhone trial. The Voice
   ...
   step joins the pair separately to preserve the complete queue in the bundle.
...
file. Test next/back/jump, pause/restart, missing or mismatched reference files,
