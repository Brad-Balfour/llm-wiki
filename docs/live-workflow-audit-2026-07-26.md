...
| Wiki maintenance 4.x                           | Source boundary, maintainer implementation, chat-mediated workflow, declarative review scenarios, and PR review guidance | Review early real maintainer PRs to validate LLM duplicate/update/link decisions before considering any auto-merge policy | Keep package scripts as agent-invoked internals. Declarative fixtures verify result recording, not maintainer judgment.                                            |
...
| Classifier calibration 5.2-5.3 / bootstrap 6.x | Exact identity and cadence policies, private append-only storage, and three validated July 20-24 backfill labels         | Off-machine private backup, measured report, explicit next-run consumption policy, and comparative regression tests       | Use the checksummed inventory in `docs/classifier-feedback-backfill-2026-07-26.md`; missing private input blocks the report rather than becoming an empty dataset. |
...
4. Back up and measure the inventoried classifier-label backfill before
   proposing any profile, prompt, routing, or classifier change.
