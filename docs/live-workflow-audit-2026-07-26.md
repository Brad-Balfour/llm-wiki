...
| Session contract 1.x                           | Bundle schema, exact/unresolved distinction, importer, real July 24 bundles                    | The enumerated missing fixture families and full playback/filename regression coverage                         | Add guards around observed failures without changing the working artifact shape.                                                         |
...
| Home intake 3.x                                | Validation, multi-bundle import, fail-soft feedback/incident persistence                       | Retryable storage for no-change/inaccessible/unresolved maintenance outcomes; remaining fixture coverage       | “Reconciliation” means validating supplied session bundles against their queue snapshots, not combining commute conversations or queues. |
| Wiki maintenance 4.x                           | Source boundary, maintainer implementation, chat-mediated user workflow                        | Inaccessible/duplicate/update/link fixtures; early PR review and any later auto-merge policy                   | Keep package scripts as agent-invoked internals. No separate command-consolidation project is needed.                                    |
...
3. Add the missing session/import fixture families that protect already
   observed failures.
