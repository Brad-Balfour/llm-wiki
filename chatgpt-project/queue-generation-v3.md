# TLDR Queue Generation Instructions — Single Playback Queue v3

Use these instructions only after the queue-generation Project has the attached
`tldr-commute-queue-v3.schema.json` file. They supersede the v1 two-array
...
must be exactly one `tldr-commute-queue.v3` object. Its leading filename date is
...
```text
Generate TLDR v3 commute queues for emails delivered on [DATE OR DATE RANGE].
