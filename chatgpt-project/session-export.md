...
For an article event, copy `source_item_id`, `title`, and `url` from the current
object in the queue. Copy Brad's exact words into `user_words`. Only connect an
...
queue file in the Project Library. Put the full JSON object from that file in
`queue_snapshot.queue` and its filename in `queue_snapshot.filename`.

If that exact file cannot be found, look at recent Project Library files. Use
another filename only if exactly one valid Queue v3 file has the original date
and newsletter. Do not use a remembered queue or one with a similar topic.
